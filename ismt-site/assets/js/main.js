...
// Highlight current nav item (Home stays active on index.html)
...
function initNewsFilter() {
  ...
  if (!select || !newsCards.length) return;
...
function initContactForm() {
  ...
  function clearErrors() {
    ...
    form.querySelectorAll('input, textarea').forEach(f => f.removeAttribute('aria-invalid'));
  }
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (!email.value.trim()) {
      ...
      errorField(email, 'Please enter a valid email address.');
      valid = false;
    }
    if (!subject.value.trim()) {
      errorField(subject, 'Subject is required.');
      valid = false;
    }
...
(function initByPage() {
  ...
  if (body.id === 'home') initCarousel();
  if (body.id === 'news') initNewsFilter();
